...
| `source`       | string |         | Filter by ingestion source (e.g. `ingest-server`, `cli`, `burp`) |
...
> **Note:** When the server is started with `--burp-bridge-url` (see [Running the Server](/server-mode/running-the-server#live-burp-bridge)), live Burp Proxy history rows are merged into this endpoint's results labelled `source: burp` — filter to just those live rows with `?source=burp`.
