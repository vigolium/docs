---
title: "Vigolium API Reference, Scan"
description: "Single-target scans, scan management, scan history, pause/resume, logs, selective record scans, and repository uploads."
icon: "bolt"
---
...
| Field         | Type   | Required | Description                                          |
|---------------|--------|----------|------------------------------------------------------|
| `http_request_base64` | string | Yes      | Base64-encoded raw HTTP request (preferred; `raw_request` is accepted as a legacy alias) |
| `http_response_base64`| string | No       | Optional base64-encoded raw HTTP response to pair with the request |
| `url`         | string | No       | URL hint when the raw request lacks scheme/host       |
...
| Field                  | Type              | Required | Description                                                                 |
|------------------------|-------------------|----------|-----------------------------------------------------------------------------|
| `targets`              | string[]          | Yes*     | Target URLs (like `-t`). Since v0.4.8 a schemeless entry (`example.com`) is normalized to `http://` by the runner, so an API caller need not prefix a hostname list. |
...
| `concurrency`          | int               | No       | Number of parallel workers (default `25`, shared with the CLI since v0.4.7) |
...
| `scanning_max_duration`| string            | No       | Global max scan duration as Go duration. Per-phase durations are derived automatically using `duration_factor` from the scanning pace config (e.g. spidering gets `0.15 × max_duration`). |
| `scope_origin`         | string            | No       | [Host scope](/native-scan/scan-scope#host-scope) mode: `all`, `relaxed`, `balanced`, `strict`. Default `balanced` since v0.4.9 (was `relaxed`). The resolved value is echoed back as `scope_origin_mode` on the scan object |
...
✦ Scan Configuration
  ...
  ℹ Speed: concurrency=25 | rate-limit=100 | max-per-host=30
  ℹ Modules: 208 active, 117 passive
```
...
> To cancel a running scan, call `POST /api/scans/:uuid/stop` (covered under [Scan Management](#post-apiscansuuidstop--stop-a-running-scan)) — there is no global `DELETE /api/scan` route.
...
| `phase`   | string |         | Filter by scan phase: `config`, `heuristics`, `harvest`, `discovery`, `spidering`, `known-issue-scan`, `dynamic-assessment` (alias `audit`), `seed`. Use `config` to retrieve the scan configuration snapshot. |
| `max_bytes` | int  | 2 MiB   | Cap on how much of `runtime.log` is read, up to 64 MiB (v0.4.2) |

<Note>
  **v0.4.2:** this endpoint returns the **last 2 MiB** of `runtime.log` by default rather than the whole file on every poll — a long-running scan's log grows without bound, and a dashboard polling it was re-reading all of it each time. Raise the window with `?max_bytes=` (hard cap 64 MiB).
</Note>
...
```json
{
  ...
  "logs": [
    ...
    },
    {
      ...
      "metadata": "{\"project_uuid\":\"my-project-uuid\",\"targets\":[\"https://example.com\"],\"strategy\":\"balanced\",\"concurrency\":25,\"rate_limit\":100,\"max_per_host\":30,\"active_modules\":208,\"passive_modules\":117,\"spidering_enabled\":true,\"discovery_enabled\":true,\"known_issue_scan_enabled\":false}",
      "created_at": "2026-02-16T15:00:00Z"
    },
    {
      "id": 3,
    ...
    },
    {
      ...
      "phase": "audit",
      "message": "phase started",
      "metadata": "{\"active_modules\":208,\"passive_modules\":117}",
  ...
  "logs": [
    {
      ...
      "phase": "config",
      "message": "scan configuration snapshot",
      "metadata": "{\"project_uuid\":\"my-project-uuid\",\"targets\":[\"https://example.com\"],\"strategy\":\"balanced\",\"scanning_profile\":\"\",\"concurrency\":25,\"rate_limit\":100,\"max_per_host\":30,\"heuristics_check\":\"basic\",\"scope_origin_mode\":\"balanced\",\"active_modules\":208,\"passive_modules\":117,\"spidering_enabled\":true,\"discovery_enabled\":true,\"known_issue_scan_enabled\":false,\"external_harvest\":false,\"skip_dynamic\":false}",
      "created_at": "2026-02-16T15:00:00Z"
    }
  ],
  "total": 1
...
| `dynamic-assessment` | Active/passive module scanning (alias: `audit`)                   |
...
Source code uploads for AI-driven analysis (swarm, autopilot, audit) are handled via the cloud-storage API. See [Storage API](/api-references/storage):
