...
| `anthropic-vertex` | `claude-opus-4-7` | GCP service-account JSON (or `$GOOGLE_APPLICATION_CREDENTIALS`) + project/location |
...
- **Cost**: LLM API calls still have token costs; `--max-commands` (autopilot) caps the iteration budget.
