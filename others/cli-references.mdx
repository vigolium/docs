...
vigolium agent autopilot -t https://example.com --source ./src --vigolium-audit=off   # disable vigolium-audit
...
Runs vigolium-audit and/or piolium back-to-back against a single source tree under one AgenticScan, with per-driver session subdirs and a post-pass findings dedup.

```bash
# Default: vigolium-audit then piolium under one AgenticScan, balanced mode
vigolium agent audit --source .

# Single driver (equivalent to agent piolium / agent vigolium-audit)
...
# Driver-specific modes (piolium=longshot/smoke, vigolium-audit=mock)
vigolium agent audit --driver piolium --source ./mono-repo --mode longshot \
  --plm-longshot-langs python,go --plm-longshot-limit 200
vigolium agent audit --driver vigolium-audit --source ./backend --mode mock
