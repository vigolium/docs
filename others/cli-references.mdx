...
| `--format <list>` | Output format(s): `console`, `jsonl`, `html`, `sqlite`, `fs` |
...
`vigolium export --format` takes a comma-separated list — `html`, `report`, `pdf`, `jsonl`, `markdown` (alias `md`), `bundle` (alias `gz`), `fs`. The database is read **once** and every format renders from that one result set, so three formats cost one query rather than three:
