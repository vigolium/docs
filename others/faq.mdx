...
<AccordionGroup>
  <Accordion title="What is Vigolium?">
    Vigolium is a high-fidelity web vulnerability scanner written in Go. It combines deterministic, module-based scanning with AI-driven agentic analysis to provide broad and deep coverage of web application security issues. It ships 325 modules (208 active, 117 passive) covering injection flaws, misconfigurations, information disclosure, authentication issues, and more.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Where did my missing-security-header findings go?">
    As of **v0.5.0**, the 14 modules tagged `hygiene` do not run below `--intensity deep`. They report a *missing best-practice control* rather than an exploitable condition — absent security headers, weak TLS protocol and cipher policy, cookie and session-cookie attributes, the CSP / HSTS / SRI / Permissions-Policy / COOP audits — and they fire on nearly every response, so a crawl of any size produced one near-identical Info/Low row per URL and buried the findings worth triaging.

    Turn them back on with `--intensity deep`, `--module-tag hygiene` (just this family, without raising every other module's aggressiveness tier), or `vigolium config set dynamic-assessment.hygiene_modules true`. The scan banner reports how many were suppressed. See the [full list](/native-scan/modules-reference#hardening-advisories-hygiene).
  </Accordion>
  <Accordion title="Why is my scan touching hosts I never named?">
    `--scope-origin` is not only a filter on new traffic — it decides which **stored** records a scan picks up. Every assessment phase resolves its work set from the database (distinct origins in the project that pass the scope matcher), and `http_records` carries no scan id, so there is no "this scan only" filter. At the default `balanced` mode, a project database holding earlier scans means every sibling host sharing the target's eTLD+1 gets re-scanned — and sorted *ahead* of the host you just named, because the work set is ordered `risk_score DESC, created_at ASC`.

    Use `--scope-origin strict` for exactly the host named, or `-S/--stateless` for a throwaway database with nothing pre-existing in it. See [Host scope](/native-scan/scan-scope#host-scope).
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Can I use a local model (Ollama, LM Studio, vLLM)?">
    Yes, use the `openai-compatible` provider, which speaks the OpenAI Chat Completions wire format. Set `custom_provider.base_url` and leave `api_key` empty for unauthenticated local servers.

    All three spellings work as of **v0.4.7**: the full endpoint (`http://localhost:11434/v1/chat/completions`), the `/v1` root (`http://localhost:11434/v1`), and a **bare host** (`http://localhost:11434`), which gets `/v1` + the endpoint appended. A bare host used to POST to the root and 404. Because the last case is a guess — a server can mount its API anywhere — the other spelling is retried once on failure and the winner is latched for the rest of the process. The same resolution now applies to `anthropic-compatible` (`/v1/messages`), which used to answer the question differently from its OpenAI sibling.
