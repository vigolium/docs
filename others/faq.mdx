...
<AccordionGroup>
  ...
  <Accordion title="What input formats can I feed a scan?">
    URLs, OpenAPI/Swagger, Postman collections, cURL commands, Burp raw requests, Burp XML exports, Burp project-config scope exports (`burpscope`), HAR captures, Nuclei JSONL output, and Deparos discovery output. Run `vigolium --list-input-mode` for every supported format with examples.
  </Accordion>
