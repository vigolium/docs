...
A **`replay` refocus**: bulk-selection filters now track `vigolium traffic`, so you can search a slice of stored traffic and re-send it (to a proxy / Burp, or straight to the wire) in one command — and payload fuzzing is consolidated into `vigolium fuzz` by removing `replay`'s `--mutate`. No module changes.
