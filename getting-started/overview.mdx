...
| **Piolium** | `vigolium agent audit --driver=piolium` | Pi-native whitebox audit driver, up to 17 phases at `deep`. Requires the `pi` runtime and the `piolium` extension installed separately. |
...
<Warning>
  Agent mode runs with **no sandbox**: the LLM has full shell, file, and network access on the host, and extensions can run arbitrary commands. Run agent mode in a disposable container or VM scoped to your engagement. See the [Security Warning](/others/security-warning) before you start.
</Warning>


### Native Scan

Deterministic, multi-phase vulnerability scanning via `vigolium scan`. Fast, modular, and repeatable, runs content discovery, browser spidering, SPA crawling, and active/passive `dynamic-assessment` phases with **302 scanner modules** (193 active, 109 passive).
