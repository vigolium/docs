...
| **Vigolium Audit** | `vigolium agent vigolium-audit` | Multi-phase whitebox security audit (lite / balanced / deep). Embedded driver, no extra install. Runs as a foreground audit or in the background alongside swarm/autopilot. |
...
- **Source-aware agentic scan**: pair `--source` with `swarm`, `autopilot`, or `vigolium-audit` for code-context-aware AI scanning and audit
