...
| **Audit** | `vigolium agent audit` | Unified driver dispatcher for whitebox security audits — runs the embedded vigolium-audit harness, the standalone piolium harness, or both side-by-side under one parent scan. Modes: lite / balanced / deep. |
| **Piolium** | `vigolium agent piolium` | Pi-native whitebox audit driver, up to 17 phases at `deep`. Requires the `pi` runtime and the `piolium` extension installed separately. |
| **Olium** | `vigolium olium` / `vigolium ol` | Interactive TUI chat or one-shot non-interactive prompt, the underlying agent runtime. |

All scan-oriented modes support `--source` for source-aware analysis and store session artifacts (plans, extensions, output) under `~/.vigolium/agent-sessions/`. See [Setting Up the Agent](/getting-started/setup-agent) for provider setup.


### Native Scan

Deterministic, multi-phase vulnerability scanning via `vigolium scan`. Fast, modular, and repeatable, runs content discovery, browser spidering, SPA crawling, and active/passive `dynamic-assessment` phases with **251 scanner modules** (154 active, 97 passive).
...
|--------------|------------|
| Install the CLI and run my first scan | [Quickstart](/getting-started/quickstart) |
| Pick the right mode for the job | [Choosing a Scan Mode](/getting-started/choosing-a-mode) |
| Run one-shot scans (CI, ad-hoc) | [Native Scan & Stateless Scanning](/getting-started/native-scan) |
| Wire up AI providers | [Setting Up the Agent](/getting-started/setup-agent) |
...
| Dive into individual scan phases | [Phases](/native-scan/phases/discovery), discovery, spidering, audit, extension |
...
| Run Vigolium as an API server | [Server Mode](/server-mode/running-the-server) |
