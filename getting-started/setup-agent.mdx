...
olium talks to a provider (Claude / OpenAI / a local model), and two specialised drivers — **archon** and **piolium** — run on top of it for source-code audits.
...
Findings land under `~/.vigolium/agent-sessions/<scan-uuid>/archon-audit/`. See [Archon Audit](/agentic-scan/archon-audit) for the full reference.
...
By default vigolium uses pi's per-user install at `~/.pi/agent`. To use a system-wide install instead, export `PIOLIUM_HOME=/opt/piolium` (or any other path). See [Piolium Audit](/agentic-scan/piolium-audit) for modes, intensity presets, and the full flag reference.
...
- [Archon Audit](/agentic-scan/archon-audit) — archon reference.
- [Piolium Audit](/agentic-scan/piolium-audit) — piolium reference.
