...
Teach your agent to drive Vigolium by installing the **`vigolium-scanner` skill** ([github.com/vigolium/skills](https://github.com/vigolium/skills)):

```bash
# Install the skill for your agent (e.g. claude-code, codex)
...
| `--full-body` | complete decoded bodies (use when writing an exploit — see [section 22](#22-show-how-to-exploit-a-finding)) |
| `--markdown` | render the finding with request/response in fenced `http` blocks |
...
<Callout icon="circle-info" color="#3B82F6" iconType="regular">
  Under `--json`, bodies are **always** bounded regardless of `-S`. Under `--markdown`, the response is only windowed with `-S`/`--stateless` + `--compact` — against your live project DB, `--markdown` renders bodies whole. So for token-safe Markdown triage, read from a stateless export (`-S --db ./scan.jsonl --compact`). An agent run (`agent autopilot|swarm|audit`) under `--json` even prints a ready-made follow-up command in its summary: `vigolium finding --agentic-scan <uuid> --json --with-records`. Full guide: [Using Vigolium in your agent](/agentic-scan/using-vigolium-in-your-agent).
</Callout>
