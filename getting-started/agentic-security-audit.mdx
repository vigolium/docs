...
- **`vigolium agent piolium`** (or `vigolium agent audit --driver=piolium`): Pi-native driver. Up to 17 phases at `deep`. Requires the `pi` runtime and the `piolium` extension. Supports any provider Pi supports, including local models.
