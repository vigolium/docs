...
The `--format` flag controls the output format. Four formats are available:
...
Results are scoped to the active project. Use `--project` to specify a project, or set a default with `vigolium project use <name>`. See the [projects documentation](/others/projects) for details on multi-tenancy and project scoping.
