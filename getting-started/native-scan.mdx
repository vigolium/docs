...
| `report` / `pdf` | `<o>.html` / `<o>.pdf` | Document-style report; `pdf` renders through headless Chrome |
| `sarif` | `<o>.sarif` | SARIF 2.1.0 for GitHub code scanning / DefectDojo / SARIF viewers; requires `-o` |
| `sqlite` | `<o>.sqlite` | Standalone DB dump (`VACUUM INTO`); requires `-S` + `-o`. Aliases `sqlite3`, `db` |
| `fs` | `<o>-traffic/`, `<o>-findings/` | Flat browsable tree; alias `file-system` |
