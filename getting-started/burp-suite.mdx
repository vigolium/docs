...
`--highlight` accepts `none`, `red`, `orange`, `yellow`, `green`, `cyan`, `blue`, `pink`, `magenta`, or `gray`. A per-request send timeout is available via `--send-timeout` on `fuzz` (capped at 2 minutes; the bridge's 30s default otherwise).
