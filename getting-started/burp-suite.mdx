...
```bash
# Replay a stored record through Burp's engine — exact bytes, no Go-client normalisation
vigolium replay --record-uuid abc12345 \
                 --burp-bridge-url http://127.0.0.1:9009 --send-via-burp

# Send a hand-crafted raw request byte-for-byte through Burp's engine
vigolium replay --raw-request-file exact.txt \
                 --burp-bridge-url http://127.0.0.1:9009 --send-via-burp

# Request smuggling / desync: force HTTP/1 so 'auto' doesn't renegotiate HTTP/2 and reframe the bytes
vigolium replay -i req.txt --burp-bridge-url http://127.0.0.1:9009 \
                 --send-via-burp --http-mode http1

# Also open the request in a Burp Repeater tab (named 'vigolium' by default)
vigolium replay --record-uuid abc12345 --burp-bridge-url http://127.0.0.1:9009 \
                 --to-repeater --repeater-tab desync

# Store the request + its response in Burp's Organizer with a note and highlight colour
vigolium replay --record-uuid abc12345 --burp-bridge-url http://127.0.0.1:9009 \
                 --to-organizer --notes 'IDOR candidate' --highlight orange

# Fuzz every payload through Burp, and push each matched anomaly into the Organizer
...
# Hand a finding's evidence request+response to Burp's Organizer for manual confirmation
vigolium finding xss --push-to-burp --burp-bridge-url http://127.0.0.1:9009

# Open a finding in a Repeater tab and re-issue it through Burp to capture a fresh response
...
<Callout icon="triangle-exclamation" color="#f59e0b" iconType="regular">
  Burp caps Repeater at ~30 tabs/minute. For a large batch of findings prefer `--push-to-burp` (Organizer, one item per finding) over `--to-repeater`.
</Callout>

`--highlight` accepts `none`, `red`, `orange`, `yellow`, `green`, `cyan`, `blue`, `pink`, `magenta`, or `gray`. A per-request send timeout is available via `--send-timeout` on `replay` and `fuzz` (capped at 2 minutes; the bridge's 30s default otherwise).
