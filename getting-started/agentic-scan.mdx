...
| Preset | Max commands | Timeout | Vigolium-audit mode (with `--source`) | Browser |
|--------|:------------:|:-------:|:------------------------------------:|:-------:|
...
vigolium agent swarm -t https://example.com --vuln-type sqli --focus "auth bypass"
```
...
vigolium agent autopilot -t https://example.com --source ~/src/your-app --vigolium-audit=deep

# Disable the audit prep
vigolium agent autopilot -t https://example.com --source ~/src/your-app --vigolium-audit=off
...
Findings ingest into the same `findings` table as native scans, tagged by source (`autopilot`, `swarm`, `archon`, `piolium`). Query them with:
