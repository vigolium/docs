---
title: "Ingesting HTTP Traffic"
description: "Get HTTP requests into Vigolium's database via API, CLI, or transparent proxy for vulnerability scanning."
icon: "upload"
---
...
3. **Transparent proxy** -Route traffic through Vigolium's built-in proxy (see [Proxy](/server-mode/proxy))
4. **Live proxy bridge** -Pull traffic straight from a running Burp Suite or Caido over the loopback bridge (see [Burp Suite](/getting-started/burp-suite) / [Caido](/getting-started/caido-plugin))
...
# Pin the scan UUID (use to group several ingests under one scan)
...
`vigolium ingest` auto-detects the content shape of stdin and `--input` files. You no longer need `-I/--input-mode` for the common formats:

| Auto-detected | Sample input |
|---|---|
| URL list | `https://example.com/api/users` (one URL per line) |
| Raw HTTP request | `POST /api HTTP/1.1\r\nHost: example.com\r\n\r\n…` |
| Burp request+response pair | concatenated raw request and response separated by the standard delimiter |
| Curl command | `curl -X POST https://example.com/api -d '…'` |

When a Burp pair already includes a response, the record is saved as-is (no live re-fetch). A short stderr preview and JSON summary report what was detected and ingested. Use `-I` only when you want to force a specific format (e.g. parsing a Postman/OpenAPI/HAR file).
...
| `har` / `http_archive` | `content` or `content_base64` | HAR file (browser/DevTools archive) |
| `nuclei-output` (alias `nuclei`) | `content` or `content_base64` | Nuclei JSONL output |
