---
title: "Stateless Scanning"
description: "Scan targets and get results in a single command without managing a persistent database, ideal for CI/CD, scripting, and quick checks."
icon: "bolt"
---
...
<Note>
  **"Stateless" is a statement about the database's lifetime, not about its absence.** Every result Vigolium produces is written through a database. Without `-S` the run persists into the project database like any other; with `-S` it runs into a throwaway file that is exported and then deleted. Dropping `-S` is therefore not a way to make a run cheaper - and under `-S` a failed export is a failed run (exit `1`, `error.code: export_failed`), because the artifact is the only copy that survives.
</Note>
...
### Reach for `-S` for isolation, not just for CI

The other half of what `-S` buys you is an **empty database**. A scan against the persisted database resolves its work set from every origin already in the project that passes the scope matcher (see [Host scope](/native-scan/scan-scope#host-scope)), so repeated scans into the default store silently grow what each run touches - and records from earlier scans sort *ahead* of the target you just named.

`-S` removes that variable entirely, which makes it the right default for a measurement run, a coverage comparison, or any scan whose result set you intend to read as "what this target looks like".

`-o` is **optional, not required**: without it the run warns that results will be discarded with the temp database and continues. The warning is suppressed under `--silent`, `--split-by-host`, `-j` and `--ci-output` - the last two stream every record to stdout as it is written, so nothing is actually lost.

<Warning>
  **A requested artifact that was never written fails the run (v0.5.0).** An export failure used to exit `0`, report `scan.finished status=completed` and leave no file. It now exits `1` with `error.code: "export_failed"`, which outranks the `--fail-on` gate - a `4` whose artifact is missing would send a CI job to read a file that is not there. Each `--format` is attempted independently, so with `--format jsonl,sqlite` one of them may still be on disk; the **Exports** summary on stderr lists what landed.
</Warning>
...
```bash
vigolium scan --stateless -t https://example.com --format sarif -o results.sarif
```

Pairs naturally with a stateless CI run — nothing is persisted, and the artifact is the upload. See [Output and Reporting](/getting-started/output-and-reporting#sarif) for the finding→schema mapping.
