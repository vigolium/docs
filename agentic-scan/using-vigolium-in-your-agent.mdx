...
Parse `items` — it is canonical. Each command also writes its historical row key (`records`, `findings`, `scans`, `rows`, `stats`) as a *deprecated alias* pointing at the same slice; don't write key fallbacks. **Assert `db_path`**: the database open order ends at one shared default file, so a fall-through silently mixes engagements. Run the `query` field rather than composing your own follow-up.
...
**`4` is not a failure.** The scan ran to completion, wrote its output, and found something at or above the threshold — the opposite outcome from `1`, where it never got that far. See the [full table](/others/cli-references#exit-codes).
