...
- **Throwaway runs:** `-S`/`--stateless` runs the whole autopilot into a throwaway temp DB (project DB untouched, like `vigolium scan -S`) and materializes `--format` outputs from it — `console`, `jsonl`, `html`, `report`, `pdf`, `sarif`, `sqlite`, `fs`, with `-o`/`--output` as the shared base (default `vigolium-result/vigolium-autopilot`). A file-producing `--format` is an error **without** `-S`; `-S` is rejected with `--db` and `--resume`, and **ignores `--db-isolate` with a warning** (a throwaway run keeps no database to merge from).
