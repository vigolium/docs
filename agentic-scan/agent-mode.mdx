...
See [Archon Audit](/agentic-scan/archon-audit).
...
See [Piolium Audit](/agentic-scan/piolium-audit).
...
The `audit` endpoint dispatches sequentially when `driver: "both"` (default) and multiplexes SSE chunks with a `driver` field bracketed by `driver_start`/`driver_end` markers when `stream: true`. See [Piolium Audit](/agentic-scan/piolium-audit#rest-api) for the full unified-driver request schema.
