...
| `--vigolium-audit` | `lite` / `balanced` / `deep` / `mock` / `off` (parallel source audit) |
...
At least one of `input`, `inputs`, `target`, `record_uuid`, or `http_request_base64` is required. See [API Reference — Agent](/api-references/agent) for the full schema.
