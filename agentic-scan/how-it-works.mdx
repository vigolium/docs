...
│  anthropic (API/OAuth/CLI/Vertex) · openai (API/codex-OAuth) ·  │
...
- **`pkg/olium/provider`**: provider dispatch (eight drivers):
  ...
  | `anthropic-cli`       | shells out to the local `claude` binary               |
...
agent:
  ...
  context_limits:
    ...
    min_risk_score: 50
  olium:
    provider: openai-compatible    # openai-codex-oauth | anthropic-api-key | openai-api-key | anthropic-oauth | anthropic-cli | anthropic-vertex | google-vertex | openai-compatible
...
CLI flags (`--provider`, `--model`, `--oauth-cred`, `--oauth-token`, `--llm-api-key`) override the config at runtime. The REST API also accepts per-request BYOK credentials.
