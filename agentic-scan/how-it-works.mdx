...
| `vigolium-audit` | Foreground vigolium-audit (multi-phase AI source-code audit).    |
...
│  query · autopilot · swarm · vigolium-audit · olium · session   │
└───────────────────────────────┬─────────────────────────────────┘
                                │
┌───────────────────────────────▼─────────────────────────────────┐
...
│     vigolium-audit (optional, foreground) → autonomous operator │
└───────────────────────────────┬─────────────────────────────────┘
                                │
┌───────────────────────────────▼─────────────────────────────────┐
...
- **`pkg/olium/provider`** — provider dispatch (five drivers):

  | Provider              | Auth source                                           |
  |-----------------------|-------------------------------------------------------|
  | `codex-oauth`         | `oauth_cred_path` (JSON from `codex login`)           |
  | `anthropic-api-key`   | `llm_api_key` or `$ANTHROPIC_API_KEY`                 |
  | `claude-oauth`        | `oauth_token` (from `claude setup-token`); falls back to `$ANTHROPIC_API_KEY` |
  | `openai-api-key`      | `llm_api_key` or `$OPENAI_API_KEY`                    |
  | `claude-code-cli`     | shells out to the local `claude` binary                |

Configured under `agent.olium` in `vigolium-configs.yaml`. Per-call deadline defaults to 10 minutes (`call_timeout_sec`).
...
A parallel **vigolium-audit** subprocess can run in the background (`cfg.Archon != ""`) when source is provided, contributing source-code audit findings without blocking the swarm.
...
├── archon-audit/           # vigolium-audit subprocess output (audit-state.json + findings)
...
├── audit-stream.jsonl      # vigolium-audit: streaming agent events
...
agent:
  ...
  context_limits:
    ...
    min_risk_score: 50
  olium:
    provider: codex-oauth          # or anthropic-api-key | openai-api-key | claude-oauth | claude-code-cli
...
CLI flags (`--provider`, `--model`, `--oauth-cred`, `--oauth-token`, `--llm-api-key`) override the config at runtime.
