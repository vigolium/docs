...
A global semaphore caps in-flight provider calls; the value comes from `agent.olium.max_concurrent` (default 4).
...
Effort level (`low`/`medium`) is picked from vigolium-audit mode: `balanced`/`deep` vigolium-audit → `medium` effort, else `low`. The operator stream is captured to `<session>/output.md`.
...
├── runtime.log             # raw runtime log mirror (for `vigolium agent log <uuid>`)
