...
| `--vigolium-audit` | `lite` (with `--source`) | Vigolium-audit mode: `lite` / `balanced` / `deep` / `mock` / `off` |
...
~/.vigolium/agent-sessions/{run-uuid}/
  ...
  └── archon-audit/ # vigolium-audit subprocess output (when --source is provided)
```
...
Vigolium-audit also runs first (foreground) when `--source` is set, freezing its findings into `archon-audit/` for the operator to consult.
