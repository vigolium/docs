...
| `--db-isolate` | false | Scan into a private temporary database, then merge results into `--db` at the end (lets parallel runs share one DB; SQLite only). Ignored with a warning under `-S`, which keeps no database to merge |
...
| `--format` | `console` | Comma-list of outputs to write from a `-S` run: `console`, `jsonl`, `html`, `report`, `pdf`, `sarif`, `sqlite`, `fs`. A file-producing format is an error without `-S`. |
...
`--format` accepts a comma-separated list of `console`, `jsonl`, `html`, `report`, `pdf`, `sarif`, `sqlite`, `fs` — exactly the same set `vigolium scan -S` accepts, from the same shared list. With more than one format, `-o/--output` is a shared **base path** and each format appends its own extension; with no `-o`, the base defaults to `vigolium-result/vigolium-autopilot`. The exported `.sqlite` reopens directly:
...
`-S` is rejected alongside `--db` and `--resume` — a database you name by hand is never silently discarded, and a resumed run restores its target, scratchpad, and prior findings from the project database, which a throwaway one does not have. `--db-isolate` is the exception: since v0.4.8 it is **ignored with a warning** rather than rejected, because its destination is implicit and `-S` keeps no database to merge from. Under a multi-app natural-language prompt, every app runs into the same scratch database and the combined result is exported once.
