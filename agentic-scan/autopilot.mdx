...
| `--max-commands` | 500 (balanced) | Max LLM turns the agent can take. Intensity preset sets this — quick=150, balanced=500, deep=1500. |
...
| `anthropic-vertex`     | `claude-opus-4-7`    | GCP service-account JSON + project/location               |
...
Prompt caching (`EnablePromptCache: true`) is set on the engine; only the Anthropic providers and the Codex OAuth provider actually emit cache markers — `openai-compatible` (including Ollama) and `openai-api-key` ignore them.
