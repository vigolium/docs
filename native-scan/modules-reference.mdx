...
| `sourcemap-ingest` | Source Map Ingest | Fetches an exposed `.map`, recovers the original pre-build source, secret-scans it and ingests the routes it reveals. Found via `sourceMappingURL`, a `SourceMap` header, or the conventional `<bundle>.map` sibling | Medium | Certain | `sourcemap`, `information-disclosure`, `javascript`, `discovery`, `spec-ingest`, `light` |
...
| `sourcemap-detect` | Sourcemap Exposure | Exposed source maps, referenced from JS or CSS `sourceMappingURL` comments, `eval` strings, or a `SourceMap` / `X-SourceMap` header. `sourcemap-ingest` then fetches and parses what this finds | Low | Firm | `info-disclosure` |
