...
description: "Reference for Vigolium's 262 scanner modules, 166 active and 96 passive, covering the OWASP Top 10 and beyond."
icon: "cubes"
---

Vigolium ships with **262 scanner modules**, 166 active and 96 passive, covering the OWASP Top 10 and beyond. Run `vigolium module ls` for the up-to-date enabled list (the registry can grow between releases without a docs refresh).

Active modules send modified requests to detect vulnerabilities; passive modules analyze existing request/response pairs without generating traffic. Recent additions include the OS command-injection suite (results/OAST/time-based), the URL-parser-confusion and routing-based SSRF family, and a cache-poisoned-DoS probe, see `vigolium module ls --tag command-injection`, `--tag ssrf`, and `--tag cpdos` to view them.
