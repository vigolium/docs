...
Swarm accepts a pre-built bundle via its own `--auth-config` flag (it skips browser/CLI auth synthesis and uses the file verbatim). Autopilot does not take a pre-built file; it prepares auth from a preflight driven by `--credentials` (plus `--auth-required` / `--requires-browser`):
...
# Autopilot prepares auth from credentials during a preflight
