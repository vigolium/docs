...
Three layers of JS analysis feed endpoints back into the discovery queue:
...
- **Source maps** (v0.4.9): An exposed `.map` is fetched and parsed, its original pre-build sources are secret-scanned, and the routes they reveal are ingested. Minification obscures identifier names, unreferenced admin routes and hardcoded credentials; the map gives them all back.

Extracted endpoints become priority-0 tasks - tested before any wordlist fuzzing.

<Note>
  **Source-map detection covers more than the `sourceMappingURL` comment.** CSS files, `eval` strings, `SourceMap` / `X-SourceMap` response headers, and the conventional `<bundle>.map` sibling that nothing links to are all checked - stripping the comment leaves the map served. A map returned as `application/octet-stream`, the static-host default for `.map`, is handled too.

  `sourcemap-detect` (passive) finds the reference; [`sourcemap-ingest`](/native-scan/modules-reference) (active) fetches, parses and feeds it back. Before v0.4.9 this chain existed but was dead code: the prefix breaker dropped referenced `.map` URLs and link extraction won the request cache, so the only parsing branch was always deduped away.
</Note>
