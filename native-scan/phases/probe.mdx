...
<Note>
  **Bare hostnames are fine.** A subdomain list with no schemes is normalized per line before the sweep starts, so `hosts.txt` straight out of a recon tool needs no pre-processing. Since v0.4.9 a schemeless probe target **resolves its scheme by TCP connect** - plus a TLS handshake on a non-standard port - rather than defaulting to `http://` and recording a redirect to the real one. See [Schemeless targets](/others/cli-references#schemeless-targets).
</Note>
...
<Warning>
  **Every mode also stops at a login/SSO wall (v0.4.9)**, whatever host the wall lives on, and records the 3xx instead of following it. A walled scan used to spend its entire budget crawling - and credential-POSTing - a third-party identity provider. Each phase reports the walls it hit, with an example target.
</Warning>
...
Since v0.4.9 the flag gates **only the hop rows**: the request/response re-pairing happens regardless, so a redirected record always names the server whose bytes it carries. Stored chains also **collapse canonical hops** - scheme upgrade, trailing slash, `www.`, default port - into their destination row and cap at **4 rows**, keeping both ends; `chain_truncated` on the record marks the ones that hit the cap, and `root_uuid` resolves the chain's first hop in O(1). See the [records API](/api-references/http-records#redirect-lineage-v049).
