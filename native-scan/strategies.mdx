---
title: "Blackbox Scanning"
description: "Test web applications from the outside without source code using crafted HTTP requests and response analysis."
icon: "chess"
---
...
<Note>
  **Since v0.4.5, `lite` also carries a pace ceiling** — concurrency 10, rate limit 20 req/s, max-per-host 10 — on top of selecting fewer phases. Before that, `lite` and `balanced` opened a crawl *identically*: a name every operator reads as "gentler on the target" was gentler in module count alone.

  The ceiling only ever **narrows**. An explicitly typed `--concurrency` / `--rate-limit` / `--max-per-host` overrules it (an operator overruling the preset is the point of having both), and a `scanning_pace` config already gentler than `lite` is not dragged up to it. `balanced` *is* the baseline and `deep` is explicitly the one that opens wide, so neither ships a ceiling.
</Note>
...
| Deparos discovery output | `-I deparos` | `vigolium scan -i deparos.jsonl -I deparos` |
| Burp scope export | `-I burpscope` | `vigolium scan -T scope.json` (content-sniffed) |
...
Sources: **Wayback Machine, CommonCrawl, AlienVault OTX, Arquivo.pt** are keyless and enabled by default; **URLScan** and **VirusTotal** join the set only once their API key is configured in `vigolium-configs.yaml` under `external_harvester.api_keys`. Configure the active set with `external_harvester.sources`.

Wayback and CommonCrawl also **mine archived response bodies**, not just the index:
...
# Custom templates directory (short alias: --templates-dir)
vigolium scan -t https://example.com --known-issue-scan-templates-dir ~/nuclei-templates/
vigolium scan -t https://example.com --templates-dir ~/nuclei-templates/
```
...
The core scanning phase (canonical name `dynamic-assessment`; aliases `audit`, `dast`, `assessment`). Runs active and passive modules against all discovered HTTP records. Enabled in all strategies.
...
<Note>
  **The pre-scan banner prints the budget the phase will actually use (v0.4.8).** An explicit `--spider-max-time` / `--discover-max-time` wins on the banner exactly as it wins in the phase. Before v0.4.8 the banner rendered the `scanning_pace` slice instead, so `--spider-max-time 5m` printed `6m0s` next to **Spidering** while the phase crawled for the 5m you asked for. Both now come from one shared budget resolver, so the two can no longer disagree.
</Note>
...
- Detection reads ordinary responses — headers on plain `200`s are enough to fingerprint **CloudFront, Cloudflare, Akamai, Imperva/Incapsula, Sucuri, and Azure Front Door**. No block or challenge is required to arm the pacing.
...
| Document report | `--format report -o report.docx` | Document-style report (Word) |
| PDF | `--format pdf -o report.pdf` | PDF report (uses headless Chrome) |
| SQLite | `--format sqlite -o scan` (needs `-S`) | Standalone DB dump (`VACUUM INTO`); aliases `sqlite3`, `db` |
...
Load JavaScript or YAML extension modules alongside or instead of built-in modules. See [Extension Scanning](/native-scan/phases/extension) for full details.
...
See [Projects](/others/projects) for the full multi-tenancy reference.
...
# Pin the scan UUID (sync results across nodes, or group several runs)
vigolium scan -t https://example.com --scan-uuid 6f1c2b90-8b1e-4a6c-9d2f-5c7a1e3b9d40
