...
All queries (findings, HTTP records, stats, scans) return data scoped to the project specified in the header. See [Projects](/projects) for the full multi-tenancy reference.
...
See [Agent Mode](/agentic-scan/agent-mode) for the full agent documentation (including autopilot, pipeline, context enrichment, and prompt templates) and the [API Reference](/api-references/agent) for request/response details.
